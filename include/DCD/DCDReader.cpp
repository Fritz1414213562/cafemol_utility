...
std::array<std::vector<float>, 3> cafemol::DCDReader::get_SnapShot_at(const std::size_t& frame_in_snapshot) {
...
std::vector<std::array<std::vector<float>, 3>> cafemol::DCDReader::get_Trajectory() {

	// read header, and store frame_num and atom_num
	read_num_frame_and_atom();
	std::vector<std::array<std::vector<float>, 3>> result(frame_num);

	for (std::size_t iframe = 0; iframe < frame_num; ++iframe) {
		result[iframe] = read_xyz(atom_num);
	}
